...
births %>%
  ...
  ggtitle("Number of Births per Month in 2000") +
  theme_bw()
```
...
births %>%
  ...
  geom_point() +
  labs(x = "Day of Week", y = "Number of Births") +
  ggtitle("Number of Births per Week in January 2000") +
  theme_bw()
```
...
births %>%
  filter(year == "2000" & month == "1") %>%
  ggplot(aes(x = day_type, y = births)) +
  geom_boxplot(outlier.shape = NA) +
  geom_jitter(alpha = 0.5, size = 2.5) +
  labs(x = "Type of Day", y = "Number of Births") +
  ggtitle("Number of Births per Day Type") +
  theme_bw()
```
...
```{r}
t_test_result <- t.test(births ~ day_type, data = births)
print(t_test_result)
```
